...
"""
1.c)
in train dataset it shows almost perfect separation, except for one sample,which lies perfectly 
on the separation line. It is optimal solution in terms of minimal distances to the separation line,
but not in terms of separability of samples.

In test dataset it gets even more obvious, that minimal distances do not nessesarily mean optimal separating
capabilities.

With outliers it only gets worse and it's a great demonstration of weakness of such classification approach
"""